...
# SAVE THE plot
ggsave("plot1.png", plot1, width = 10, height = 10, units = "cm")
...
# save the plot
ggsave("plot2.png", plot2, width = 10, height = 10, units = "cm")
...
ggsave("plot3.png", plot3, width = 10, height = 10, units = "cm")
...
ggsave("plot4.png", plot4, width = 10, height = 10, units = "cm")


plot5<-ggplot(new_ames, aes(x = garage_cars, y = sale_price, size = sale_price)) +
  geom_point(alpha = 0.7, color = "#ff0000") +
  labs(title = "Bubble Plot: stories vs. Sale Price", x = "stories", y = "Sale Price", size = "Sale Price")
ggsave("plot5.png", plot5, width = 10, height = 10, units = "cm")


plot6<-ggplot(new_ames, aes(x = house_quality, y = sale_price)) +
  geom_line(color = "blue") +
  labs(title = "Line Plot: Zone vs. Sale Price", x = "zone", y = "Sale Price")
ggsave("plot6.png", plot6, width = 10, height = 10, units = "cm")


grid_arranged <- grid.arrange(plot1, plot2, plot3, plot4, plot5, plot6, ncol = 3)
print(grid_arranged)
...
test_data$predicted_sale_price <- predict(model1, newdata = test_data)
r_squared_test <- 1 - mean((test_data$sale_price - test_data$predicted_sale_price)^2) / var(test_data$sale_price)
...
r_squared <- summary_model7$r.squared
test_data$predicted_sale_price7 <- predict(model7, newdata = test_data)
r_squared_test <- 1 - mean((test_data$sale_price - test_data$predicted_sale_price7)^2) / var(test_data$sale_price)
cat("R-squared value for test data:", r_squared_test, "\n") # higher r_squared_test = better accuracy
